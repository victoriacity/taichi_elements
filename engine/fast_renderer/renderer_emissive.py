...
@ti.func
def shade_flat_e(color):
    return color

@ti.func
def sample_floor_e(floor_height, rayorig, raydir):
    raylength = (floor_height - rayorig[1]) / raydir[1]
    intersect = rayorig + raylength * raydir
    # checkerboard texture
    floor_color = ti.Vector([0.06, 0.1, 0.1])
    col = shade_flat_e(floor_color)
    return col, intersect

@ti.func
def sample_sky_e(rayorig, raydir):
    l = max(0, raydir[1])
    return ti.Vector([0.02, 0.05, 0.08]) * (1 - l) ** 3 + ti.Vector([0.06, 0.1, 0.1]) * (1 - (1 - l) ** 3)
    ...
    def shade_particles(self, camera: ti.template()):
        ...
        for I in ti.grouped(camera.img):
            ...
            if camera.weight[I] > 0:
                ...
                #camera.img[I] += ti.Vector([1.0, 1.0, 1.0]) * smoothstep(min(camera.weight[I] * 0.2, 1))# emmisive(color)
                # reflection 
                refldir = viewdir - 2 * viewdir.dot(normal) * normal
                if refldir[1] > 0:
                    camera.img[I] += min(0.05, refldir[1] * 5) * sample_sky_e(rayorig, refldir)
                else:
            ...
            if rayorig[1] > floor_height and viewdir[1] < 0:
                # shade floor
                col, intersect = sample_floor_e(floor_height, rayorig, viewdir)
                camera.img[I] += col
            else:
                camera.img[I] += sample_sky_e(rayorig, viewdir)
            # tone mapping
